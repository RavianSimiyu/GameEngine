...
void append_to_log_file(const char* message){
    if(state_ptr && state_ptr->log_file_handle.is_valid){
        // Since the message already contains '\n' , just write the bytes directly.
...
b8 initialize_logging(u64* memory_requirement, void* state){
    *memory_requirement = sizeof(logger_system_state);
    ...
    KINFO("Logging subsystem initialized.");
...
void log_output(log_level level, const char* message, ...){
    // TODO: These string operations are all prety slow. This needs to be 
    // moved to another thread eventually, along with the fie writes, to
	...
	//NOTE: Oddly enough , Ms's headers override the GCC/Clang va_list type with a "typedef char* va_list" 
    ...
    // Queue a copy to written to the log file.
    append_to_log_file(out_message);
}


void report_assertion_failure(const char* expression,const char* message, const char* file, i32 line) {
	log_output(LOG_LEVEL_FATAL, "Assertion Failure: %s, message: %s, in line: %s, line: %d\n", expression, message, file, line);
}
