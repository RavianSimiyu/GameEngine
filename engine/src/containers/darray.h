...
// NOTE: could also use __auto_type for temp above, but intellisense
// for VScode flagsit as an unknown type. typeof() seems to 
// work just fine, though. Both are GNU extensions.

#define darray_pop(array, value_ptr) _darray_pop(array, value_ptr)

#define darry_insert_at(array, index, value) {                   \
    typefof(value) temp = value;                                 \
    array = _darray_insert_at(array, index, &temp);              \
    }

#define darray_pop_at(array, index, value_ptr) _darray_pop_at(array, index, value_ptr)

#define darray_clear(array) _darray_field_set(array, DARRAY_CAPACITY)
