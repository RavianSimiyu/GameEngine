...
b8 platform_startup(
	...
	//XCB_CW_BACK_PIXEL = filling the window bg wih a single colour.
	...
	while((event = xcb_poll_for_event(state->connection))){
		...
		switch(event->response_type & ~0x80){
			...
			case XCB_BUTTON_RELEASE:{
                ...
                buttons mouse_button = MOUSE_BUTTON_MAX;
...
void platform_get_required_extension_names(cons char*** names_darray) {
    darray_push(*names_darray, "VK_KHR_xcb_surface");   // VK_KHR_xlib_surface?
}

// Surface creation for Vulkan
b8 platform_create_vulkan_surface(
        struct platform_state* plat_state,struct vulkan_context* context) {
    // Simply cold-cast to the known type.
    internal_state *state = (internal_state *)plat_state->internal_state;
